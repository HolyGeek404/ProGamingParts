class Order {
    ...
    createOrder = () => {
        ...
        $(".product").each(function () {
            ...
            let type = $(this).attr("data-bind-Type");
            let product = {
        ...
        $.ajax({
            url: "/CartApi/CreateOrder",
            type: "POST",
            headers: { 'RequestVerificationToken': $('input[name="__RequestVerificationToken"]').val() },
            data: formData,
            success: function (result) {
                if (result.success) {
                    $("#orderFailMsg").attr("hidden", true);
                    $("#orderSuccessMsg").removeAttr("hidden");
                    setTimeout(function () {
                        window.location.href = "/User";
                    }, 2000);
                } else {
                    $("#orderSuccessMsg").attr("hidden", true);
