...
class CoolerFilter {
    constructor(quickFilter, categoryType) {
        ...
        $('input[type="checkbox"][name="CoolerTypes"][value="' + quickFilter + '"]').prop('checked', true);
        $(".Filter").on("click", () => {
    ...
    FilterHeaders(selectedFilters) {
        ...
        if (selectedFilters.length > 0) {
            ...
            if (selectedFilters.filter(obj => obj.Name == 'CoolerTypes').length > 0) {
                const CoolerTypes = selectedFilters.filter(obj => obj.Name == "CoolerTypes");
                const values = CoolerTypes.map(obj => obj.Value);
                const stringToDisplay = values.join(', ');
                $(".filters").append('<div class="rounded d-inline-block p-1 m-1 bg-light"><span class="text-dark font-weight-bold">Producent: </span>' + stringToDisplay + '</div>');
            }
