...
class Order {
    ...
    private postalCodeRegex: RegExp = /^\d{2}-\d{3}$/;
    ...
    validateDataBeforeSubmitting = (): void => {
    ...
    private gatherSubmitUserData = (): void => {
        const userData = new FormData(document.querySelector("#userDataForm"));
        const name = userData.get("name").toString();
        const surname = userData.get("surname").toString();
        const street = userData.get("street").toString();
    ...
    private validatePayAndShippingForm = (): boolean => {
        return this.validatePayForm() && this.validateShippingForm();
    }

    private validateUserDataForm = (): boolean => {
    ...
    private postalCodeValidation = (postalCode: string): boolean => {
    ...
    private blockNumValidation = (blockNum: string): boolean => {
        if (blockNum.trim() == "") {
    ...
    private flatValidation = (flat: string): boolean => {
        if (flat.trim() == "") {
    ...
    private streetValidation = (street: string): boolean => {
        if (street.trim() == "") {
    ...
    private cityValidation = (city: string): boolean => {
        if (city.trim() == "") {
    ...
    // pay form validation 
    private validatePayForm = (): boolean => {
        return $('input[name="payOptionGroup"]:checked').val() !== undefined;
    }

    private validateShippingForm = (): boolean => {
